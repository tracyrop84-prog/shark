...
function selectThemeParkRides(displayRideDetais) {
  ...
  if (ride.name === "Flatacoaster") {
    ride.description =
      "Flatacoaster is the tallest and quickest roller coaster in the world! It's Flatironland's signature ride! It's so fast, you'll be screaming 'FLATIRON!' in no time!";
  } else if (ride.name === "Flata-Ferris Wheel") {
    ride.description =
      "Roller Coasters too scary for you? Enjoy a relaxing ride and some lovely views on the Flata-Ferris Wheel!";
  } else if (ride.name === "Flatburger bumper cars") {
    ride.description =
      "Ride in our Flatburger bumper cars designed in the shape of burgers! You even get a free burger for riding!";
  }

  displayRideDetails(ride);
}
selectThemeParkRides((ride) => {
  alert(`You selected the ${ride.name} ride! Here's the ride description: ${ride.description}
You selected the ${ride.name} ride! Here's the ride description: ${ride.description}`);
});
